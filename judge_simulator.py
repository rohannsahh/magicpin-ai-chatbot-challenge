...
LLM_API_KEY = "sk-db9dbe5033e74b35a755b5f0ef365231"
            ...
            for attempt in range(4):
                try:
                    ...
                    if "429" in msg or "Too Many" in msg or "RESOURCE_EXHAUSTED" in msg:
                        wait = [10, 20, 40][attempt] if attempt < 3 else 40
                        print_warn(f"LLM 429 — waiting {wait}s before retry {attempt+1}/3")
                        time.sleep(wait)
                    else:
                        raise
            if response is None:
...
def main():
    ...
    # Test LLM connection (retry on 429)
    ...
    for _attempt in range(5):
        try:
            ...
            if "429" in msg or "Too Many" in msg:
                wait = [15, 30, 60, 90, 120][_attempt] if _attempt < 5 else 120
                print_warn(f"LLM 429 on test — waiting {wait}s before retry {_attempt+1}/5")
                time.sleep(wait)
            else:
                ...
                sys.exit(1)
    if not test_ok:
        print_fail("LLM connection failed after retries")
        sys.exit(1)
